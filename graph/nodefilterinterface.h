...
namespace Nepomuk
{
    ...
    class NodeFilterInterface : public ChildQueryInterface
    {
        ...
        protected:
            /*! \brief Main method
             * This is the method that implementations should overload. The
             * idea is easy - the children() function will return valid iterator (
             * retrieved with subQuery ) for Soprano::Node node if 
             * isOk(model,node) == true, and invalid -
             * if isOk(model,node) == false ( thus marking node as leaf )
             */
