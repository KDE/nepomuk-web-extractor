...
void Nepomuk::WebExtractorScheduler::readConfig()
{
    ...
    */
    ...
    foreach(const QString  & catname, cats) {
        ...
        if(!params.hasAnyExecutive()) {
            kDebug() << "Category " << catname << " has no assigned DataPP";
            continue;
        } else {
            kDebug() << "Category " << catname << "has " << params.executiveCount() << "assigned DataPP";
        }

        //q = m_conf->queryPrefix(catname);
        NQ::Term qt = m_conf->query(catname);

        NQ::Query aq = Nepomuk::WebExtractorQueries::ask_query(
                           qt
                       );

        kDebug() << "Category " << catname << " has query: " << aq.toSparqlQuery(NQ::Query::CreateAskQuery);

        Soprano::QueryResultIterator it =
            Nepomuk::ResourceManager::instance()->mainModel()->executeQuery(
                aq.toSparqlQuery(NQ::Query::CreateAskQuery), Soprano::Query::QueryLanguageSparql
            );
        ...
        m_askQueries[catname] = aq/*.toSparqlQuery(NQ::Query::CreateAskQuery)*/;
...
bool Nepomuk::WebExtractorScheduler::addToQueue(const QString & name)
{
    ...
    WebExtractorCategoryScheduler * sh = m_categories[name];
...
void Nepomuk::WebExtractorScheduler::start()
{
    ...
    QHash<QString, NQ::Query>::iterator qit = m_askQueries.begin();
