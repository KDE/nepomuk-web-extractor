...
QList<QWidget*> CategoryPluginItemDelegate::createItemWidgets() const
{
    ...
    */
    PluginItemWidget * w = new PluginItemWidget();
    //ui->setupUi(w);
    ...
    connect(w->descr_button, SIGNAL(clicked()), this, SLOT(descriptionToggled()));
    connect(w->scaleCoff, SIGNAL(valueChanged(double)), this, SLOT(coffChanged(double)));
...
{
    PluginItemWidget * w =    qobject_cast<PluginItemWidget*>(widgets.at(0));
    w->resize(m_sizeHint);
