...
{
    public:
        Private(const ExecutiveKeeper & dataPPKeeper, ND::DecisionFactory * fact);
        ~Private();
    public:
        //int tmp_count;
...
void Nepomuk::WebExtractor::ResourceAnalyzer::Private::enqueue(const Executive* pp)
{
        ExecutiveWrapper * dpp = m_dataPPKeeper[pp];
        Q_CHECK_PTR(dpp);
        m_queue.enqueue(dpp);
...
void NW::ResourceAnalyzer::pluginPackFinished()
{
    ...
    QSet< const ND::DecisionAuthor*> set = d->m_decisions.filterObsolete();
    if(set.size()) {
        ...
        foreach( const ND::DecisionAuthor* executive, set)
        {
             d->enqueue((const Executive*)(executive));
        }
    }
...
ND::DecisionList NW::ResourceAnalyzer::decisions() const
{
